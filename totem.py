...
def make_totem(username, use_outer_layer=True):
    ...
    right_arm_warped.paste(right_arm.crop((0, 1, 3, 2)), (3, 1))
    right_arm_warped.paste(right_arm.crop((0, 0, 1, 1)), (5, 0))
    ...
    if is_old_skin: #time saver
        ...
        left_arm_warped.paste(ImageOps.mirror(
            ImageOps.mirror(
                left_arm.crop((0, 4, 4, 5))
            ).resize((5, 1))
        ), (0, 4))
